...
class MMDEMLP(MLP):
    ...
    def __init__(self, input_size, hidden_layer_size, output_size, layer_norm, drop_out, drop_out_p, bias,
        ...
        super(MMDEMLP, self).__init__(input_size, hidden_layer_size, output_size, layer_norm, drop_out, drop_out_p,
                                      bias)
...
class MMDEMNIST(MLP):
    """
    MMDEMNIST  is an extension of the base MLP (Multi-Layer Perceptron).
    ...
    def __init__(self, input_size, hidden_layer_size, output_size, layer_norm, drop_out, drop_out_p, bias,
        ...
        # input_size = 5408
        # Initialize the base MLP
        super(MMDEMNIST, self).__init__(input_size, hidden_layer_size, output_size, layer_norm, drop_out, drop_out_p,
                                        bias)
    ...
    def forward(self, x, y) -> torch.Tensor:
        ...
        if len(x.shape) > 2 or len(y.shape) > 2:
            if self.flatten:
                ...
                # drop a dimension
                y = torch.squeeze(y, 1)
                for i in range(num_samples):  # without the rotation at 360 degrees
                    g_y += self.model(torch.flatten(y[..., i], start_dim=1)) / num_samples
                g_x = self.model(torch.flatten(x, start_dim=1))
        else:
            # If tensors are two-dimensional
            g_x = self.model(x)
            g_y = self.model(y)
