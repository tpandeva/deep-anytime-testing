...
class SwapMLP(MMDEMLP):
    ...
    def __init__(self, input_size, hidden_layer_size, output_size, batch_norm, drop_out, drop_out_p, bias, full_dim):
        ...
        # Initialize base MLP
        super(MMDEMLP, self).__init__(input_size, hidden_layer_size, output_size, batch_norm,
                                      drop_out, drop_out_p, bias, full_dim)

    def forward(self, x, swap_operator) -> torch.Tensor:
        """
        Forward pass for the MMDEMLP model. Computes the output based on inputs x and y.
        ...
        - x (torch.Tensor): First input tensor.
        - y (torch.Tensor): Second input tensor.
        - swap_operator (SwapOperator): The swap operator.

        Returns:
        - output (torch.Tensor): The output of the forward pass.
        """
        y = swap_operator.compute(x)
        x = torch.transpose(x, 1, 2).reshape(-1, 2 * self.operator.p) # flatten? or squeeze?
        y = torch.transpose(y, 1, 2).reshape(-1, 2 * self.operator.p)
        # Compute the outputs from the base MLP model for x and y
        g_x = self.model(x) if self.full_dim else self.model(x[:, :self.input_size])
        g_y = self.model(y) if self.full_dim else self.model(y[:, :self.input_size])
