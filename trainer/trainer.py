...
class TrainerMMDE(Trainer):
    ...
    def load_data(self, seed, mode= "train"):
        ...
        if mode in ["train", "val"]:
            data_loader = DataLoader(data, batch_size=self.bs, shuffle=True)
        else: data_loader = DataLoader(data, batch_size=9, shuffle=True)
        return data, data_loader

    def train_evaluate_epoch(self, loader, mode="train"):
        """Train/Evaluate the model for one epocj and log the results."""
        aggregated_loss = 0
        mmde_mult,mmde_batch, mmde_mean = 1,1,0
        num_samples = len(loader.dataset)
        for i, (z, tau_z) in enumerate(loader):
            z = z.to(self.device)
            tau_z = tau_z.to(self.device)
            ...
            mmde_batch *= torch.exp(out).mean()
            if mode == "train":
                self.optimizer.zero_grad()
                loss.backward()
                self.optimizer.step()
        self.log({f"{mode}_eval_mult": mmde_mult.item(),f"{mode}_eval_log": -aggregated_loss.item(),f"{mode}_eval_mean": mmde_mean.item(), f"{mode}_eval_batch": mmde_batch.item(),f"{mode}_loss": aggregated_loss.item()/num_samples})
        return aggregated_loss/num_samples, mmde_mult, mmde_batch, -aggregated_loss
